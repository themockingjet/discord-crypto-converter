...
module.exports = new Command({
	...
	async run(message, args, client) {
		if (!args[1]) return message.channel.send("Invalid command. Type `!axs<space><amount>` for more info.");
		if (isNaN(parseInt(args[1]))) return message.channel.send("Invalid amount. Type `!axs<space><amount>` for more info.");
		if (args[2]) return message.channel.send("Invalid command. Type `!axs<space><amount>` for more info.");
